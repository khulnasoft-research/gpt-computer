...
from gpt_computer.core.default.paths import ENTRYPOINT_FILE, PREPROMPTS_PATH
...
from gpt_computer.core.linting import Linting
...
class TestGenCode:
    ...
    @pytest.mark.asyncio
    async def test_raises_type_error_if_keys_not_strings_or_path_objects(self):
        # Mock AI class
        class MockAI:
            async def start(self, sys_prompt, user_prompt, step_name):
                return [SystemMessage(content=factorial_program)]
    ...
    @pytest.mark.asyncio
    async def test_raises_type_error_if_values_not_strings(self):
        # Mock AI class
        class MockAI:
            async def start(self, sys_prompt, user_prompt, step_name):
                return [SystemMessage(content=factorial_program)]
    ...
    @pytest.mark.asyncio
    async def test_raises_key_error_if_file_not_exist_in_database(self):
        # Mock AI class
        class MockAI:
            async def start(self, sys_prompt, user_prompt, step_name):
                return [SystemMessage(content=factorial_program)]
...
class TestGenEntrypoint:
    class MockAI:
        ...
        async def start(self, system, user, step_name):
            return [SystemMessage(content=self.content)]

    #  The function receives valid input and generates a valid entry point script.
    @pytest.mark.asyncio
    async def test_valid_input_generates_valid_entrypoint(self):
        # Mock AI class
        ...
        # Act
        preprompts_holder = PrepromptsHolder(PREPROMPTS_PATH)
        entrypoint_code = await gen_entrypoint(
...
"""
    ...
    @pytest.mark.asyncio
    async def test_empty_codebase_returns_empty_entrypoint(self):
        # Arrange
...
class TestImprove:
    @pytest.mark.asyncio
    async def test_improve_existing_code(self, tmp_path):
        # Mock the AI class
        ai_patch = """
...
"""
        ...
        class MockAI:
            async def next(self, messages, prompt=None, *, step_name):
                return [SystemMessage(content=ai_patch)]
